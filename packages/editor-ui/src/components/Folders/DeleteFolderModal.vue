...
const folderToDelete = computed(() => {
	...
	return foldersStore.foldersCache[props.activeId];
});
...
const folders = computed(() => {
	return Object.values(foldersStore.foldersCache);
});
		...
		<template #content>
			<div>
				...
				<div v-else :class="$style.content">
					<div>
						<n8n-text color="text-base">{{
							i18n.baseText('folders.delete.confirmActionAfterDelete')
						}}</n8n-text>
					</div>
					<el-radio
						v-model="operation"
					...
					>
						...
						>
							<n8n-input
								...
								:placeholder="i18n.baseText('folders.delete.confirm.title')"
							/>
						</n8n-input-label>
					</div>
				</div>
